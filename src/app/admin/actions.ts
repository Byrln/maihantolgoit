...
import { clearUserSession, getCurrentUser, requireAdmin, setUserSession } from "@/lib/auth";
...
export async function saveUser(formData: FormData) {
  ...
  if (id) {
    await prisma.user.update({
      where: { id },
      data: { email, username, name: name || null, role },
    });
  } else {
    const password = generatePassword();
    const user = await prisma.user.create({
      data: { email, username, passwordHash: hashPassword(password), name: name || null, role },
    });

    revalidatePath("/admin/users");
    redirect(`/admin/users?created=1&login=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}&user=${encodeURIComponent(user.name || user.email)}`);
  }
...
export async function uploadMedia(formData: FormData) {
  ...
  if (!media) {
    throw new Error("Зураг сонгоно уу.");
  }
